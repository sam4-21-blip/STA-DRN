...
import numpy as np
import pandas as pd
from TSSTANet.tsstanet import sanet
...
Net = sanet(layers=[2, 2, 2, 2], in_channels=3, num_classes=1, k=2, features=features)
Net = Net.to(DEVICE)
...
for epoch in range(EPOCHS):
    ...
    with open('training_log_sanet_lr001.csv', 'a') as f:
        f.write(f'{epoch},{mean_mae_loss},{mean_rmse_loss}\n')

    if mean_mae_loss < best_MAE:
        best_MAE = mean_mae_loss
        torch.save(Net.state_dict(), './model/best_sanet_lr001.pth')
